...
def smooth(input_vec, window=10):
...
def plot_from_txt(data_path, iters_path, fig_texts):
    ...
    for j in range(0, len(y)):
        y[j] = smooth(y[j], window=25)
        ax.plot(x[j], y[j], linestyle=lines_array[0], marker=markers_array[j+2], linewidth=10, markersize=20)
    ax.set(xlabel=fig_texts[0], ylabel=fig_texts[1], title=fig_texts[2])
    ax.grid()
    fig.savefig(fig_texts[4])
    fig.savefig(fig_texts[5])
    plt.legend(fig_texts[3], fontsize=40)
    plt.show(block=True)
