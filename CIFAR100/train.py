...
from data_loader import get_data_loader_custom
...
def main(args):
    ...
    while k < len(instructions):
        if instructions[k][0] == 'Pretrain':
            ...
            if len(tr_cls) > 75:
                save_folder = 'CIFAR100_Simulations/Folder_Pretrain_Too_Long' + str(tr_cls[:5]) + str(tr_cls[95:-1]) + instructions[k][1]
            else:
            ...
            for it in range(len(indices)):
                tr_cls.append(args.train_classes[indices[it]])
            if len(tr_cls) > 75:
                save_folder = 'CIFAR100_Simulations/Folder_EdgeOnly_Too_Long' + str(tr_cls[:5]) + instructions[k][1]
            else:
                save_folder = 'CIFAR100_Simulations/Folder_EdgeOnly_' + str(tr_cls) + instructions[k][1]
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
                continue
            ...
            for it in range(len(indices)):
                tr_cls.append(args.train_classes[indices[it]])
            if len(tr_cls) > 75:
                save_folder = 'CIFAR100_Simulations/Folder_FullData_Too_Long' + str(tr_cls[:5]) + instructions[k][1]
            else:
                save_folder = 'CIFAR100_Simulations/Folder_FullData_' + str(tr_cls) + instructions[k][1]
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
                continue
            ...
            save_folder = 'CIFAR100_Simulations/Folder_' + str(instructions[k][2])
            if os.path.exists(save_folder):
                ...
                for ind in range(len(instructions[k][2])):
                    ...
                    if type(path) is str:
                        ...
                        dataset.append(gan.generate(gen=G_prev[ind], samp_num=args.sample_number, iter=0,
                                                    fld_str='CIFAR100_Simulations/data_folder'+str(instructions[k][3])
                                                            + str(ind)))
                    else:
        ...
        elif instructions[k][0] == 'Aggregate':
            tr_cls = []
            for kk in range(len(instructions[k][2])):
                tr_cls.append(instructions[k][2][kk])
            save_folder = 'CIFAR100_Simulations/Folder_Aggregate_' + str(instructions[k][2])
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
                continue
                ...
                # Perform Training
                d_mod = [CNN_Dis_gp(3)]
                d_opt = [optim.Adam(d_mod[kk_].parameters(), lr=args.learning_rate, betas=(args.beta_1, args.beta_2))]
                # Create temporary generator model and optimizers. These won't be used in training.
                g_mod = CNN_Gen_gp(3)     # Not used due to transfer learning
                g_opt = optim.Adam(g_mod.parameters(), lr=args.learning_rate, betas=(args.beta_1, args.beta_2))
                # Create training object
                ...
                for ind in range(len(instructions[k][2])):
                    ...
                    if type(path) is str:
                        checkpoint = torch.load(path)
                        G_prev.append(checkpoint['modelG'])
                        dataset.extend(gan.generate(gen=G_prev[ind], samp_num=int(args.sample_number/2), iter=0,
                                                    fld_str='CIFAR100_Simulations/data_folder'+str(instructions[k][3])
                                                            + str(ind)))
                    else:
                        G_prev.append(CNN_Gen_gp(3))
            ...
            if len(tr_cls) > 75:
                ...
                save_folder = 'CIFAR100_Simulations/Folder_Transfer_' + str(tr_cls) + instructions[k][1] \
                              + str(instructions[k][4])
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
            ...
            if len(edge_cls) > 75:
                save_folder = 'CIFAR100_Simulations/Folder_BaryTransfer_Too_Long' + str(edge_cls[:5]) \
                              + instructions[k][1] + str(instructions[k][4])
            else:
                save_folder = 'CIFAR100_Simulations/Folder_BaryTransfer_' + str(edge_cls) + instructions[k][1]\
                              + str(instructions[k][4])
            if os.path.exists(save_folder):
                # This model has already been trained. Skip to next item in args.train_classes
                k = k + 1
                ...
                dataset.append(gan.generate(gen=G_prev_data, samp_num=args.sample_number, iter=0,
                                            fld_str='CIFAR100_Simulations/data_folder'+str(instructions[k][4])))
            ...
            if len(bary_cls) > 75:
                save_folder = 'CIFAR100_Simulations/Folder_ModelAverage_Too_Long' \
                              + instructions[k][1] + str(instructions[k][3])
            else:
                save_folder = 'CIFAR100_Simulations/Folder_ModelAverage_' + instructions[k][1]\
...
if __name__ == '__main__':
    simulation_dir = 'CIFAR100_Simulations'
